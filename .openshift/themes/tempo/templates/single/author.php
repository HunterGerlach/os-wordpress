<?php
	if( !tempo_options::get( 'post-author-box' ) )
		return;
	...
	if( !empty( $website ) ){
		echo '<a href="' . esc_url( $website ) . '" target="_blank">' . esc_url( $website ) . '</a>';
	}
	else{
		echo '<a href="' . esc_url( get_author_posts_url( $post -> post_author ) ) . '">' . esc_url( get_author_posts_url( $post -> post_author ) ) . '</a>';
	}
